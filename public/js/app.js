document.addEventListener("readystatechange", function () {
  if (document.readyState === "complete") {
    ...
    Array.prototype.forEach.call(
      document.querySelectorAll(".show_file_button"),
      (e) => {
        e.addEventListener("click", (b) => {
          b.target.style.display = "none";
          b.target.parentElement.querySelector("strong").style.display =
            "block";
          b.target.parentElement.querySelector('[type="file"]').style.display =
            "block";
          b.preventDefault();
          b.stopPropagation();
        });
    ...
    //
    Array.prototype.forEach.call(
      document.querySelectorAll(".remove_image_button"),
      (e) => {
        e.addEventListener("click", (b) => {
          let form =
            b.target.parentElement.parentElement.parentElement.parentElement;
          form.querySelector('[name="field_to_delete"]').value =
            b.target.parentElement.querySelector('[name="field_name"]').value;
          form.method = "get";
    ...
    //
    Array.prototype.forEach.call(
      document.querySelectorAll("#submit_create"),
      (e) => {
        e.addEventListener("click", (b) => {
    ...
    if (
      ...
      "si" == document.querySelector("#tabs").value
    ) {
      const tabs = [];
      for (let i = 1; i <= 9; i++) {
      ...
      tabs.forEach((tab, index) => {
        ...
        button.addEventListener("click", (e) => {
          ...
          document
            .querySelector(".iwt-admin")
            ?.classList.toggle(
              "hotpoints-editor-opened",
              e.target.dataset.target === "tab-5",
            );
          //
    ...
    if (document.getElementById("add_user_project")) {
      document
        .getElementById("add_user_project")
    ...
    if (document.querySelector("#detection_objects")) {
      document
        .querySelector("#detection_objects")
        .addEventListener("change", function (e) {
          lanza_ajax_deteccion(e.target);
        });
      document
        .querySelector("#distance_frames")
        .addEventListener("change", function (e) {
    ...
    if (document.querySelector("#ai-machine-detection")) {
      ...
      if (btnCancel) {
        btnCancel.addEventListener("click", () => {
          ...
          document.querySelector("#ai-machine-detection").style.display =
            "block";
      ...
      updateCount();
      document
        .querySelector("#ai-machine-detection")
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      if (!Array.isArray(data.detections) || data.detections.length === 0) {
        container.innerHTML =
          "<p>No detections were found for this object.</p>";
        return;
      }
      ...
      html +=
        '</tbody></table><button id="vincular" class="btn btn-primary" disabled style="display:none;">Link</button>';
      if (!Array.isArray(lista) || lista.length === 0) {
        container.innerHTML =
          "<p>No products were found for this detection.</p>";
      } else {
        container.innerHTML = html;
      }
    ...
    .catch((err) => {
      ...
      document.querySelector("#detections_table").innerHTML =
        "<p>Error cargando las detecciones.</p>";
    });
}
...
function check_selections() {
  ...
  if (document.querySelector("#vincular")) {
    document.querySelector("#vincular").addEventListener("click", function () {
      const projectId = document.querySelector("#project_id").value;
      const detection_id = document.querySelector(
        "#datision_detection_id",
      ).value;
...
function eventDetectedItem() {
  ...
  group.forEach((row) => {
    row.addEventListener("click", (original) => {
      ...
      console.log(
        "Video time position: " +
          frame / document.querySelector("#video_fps").value,
      );
...
function showUserModal() {
  ...
  if (!modal) {
    ...
    document.getElementById("cancel_add_user").addEventListener("click", () => {
      ç;
      modal.remove();
    });

    document
      .getElementById("confirm_add_user")
    ...
    document
      .getElementById("user_select_modal")
...
function ajax_ia() {
  ...
  const collectClasses = () =>
    Array.from(
      document.querySelectorAll('input.class-item[name="classes[]"]:checked'),
    ).map((el) => el.value);

  const postJSON = async (
    url,
    body,
    { headers = {}, sameOrigin = false, timeout = 60000 } = {},
  ) => {
  ...
  btn.addEventListener("click", async (e) => {
    ...
    try {
      ...
      if (Number.isNaN(threshold))
        throw new Error("Missing or incorrect data-secs.");
      ...
      try {
        ...
        if (taskId) {
          ...
          const resultData =
            typeof resultResp === "string"
              ? JSON.parse(resultResp)
              : resultResp;
        ...
        console.warn(
          "[AI-LAUNCHER] Progress check failed:",
          err2?.message || err2,
        );
      }
...
function proyectos_objetos() {
  if (document.querySelector("#chage_state")) {
    ...
    //
    Array.prototype.forEach.call(
      document.querySelectorAll("#close_window"),
      (e) => {
        e.addEventListener("click", (elem) => {
          elem.preventDefault();
          elem.target.parentElement.style.display = "none";
          elem.target.parentElement.parentElement.classList.remove(
            "wide_td_price",
          );
        });
      },
    ...
    //
    Array.prototype.forEach.call(
      document.querySelectorAll("#chage_state"),
      (e) => {
        e.addEventListener("click", (elem) => {
          elem.preventDefault();
    ...
    // Aquí se actualiza
    Array.prototype.forEach.call(
      document.querySelectorAll("#change_state_option"),
      (e) => {
        e.addEventListener("click", (elem) => {
          elem.preventDefault();
          elem.target.parentElement.style.display = "none";
          elem.target.parentElement.parentElement.querySelector("a").innerHTML =
            elem.target.innerHTML;
          elem.target.parentElement.parentElement.parentElement.dataset.updated =
            "1";
    ...
    //
    Array.prototype.forEach.call(
      document.querySelectorAll("#chage_price"),
      (e) => {
        e.addEventListener("click", (elem) => {
          elem.preventDefault();
    ...
    Array.prototype.forEach.call(
      document.querySelectorAll("#change_price_option"),
      (e) => {
        e.addEventListener("click", (elem) => {
          elem.preventDefault();
          elem.target.parentElement.querySelector("#price_per_second").value =
            elem.target.dataset.price;
        });
      },
    );
    // Aquí se actualiza
    Array.prototype.forEach.call(
      document.querySelectorAll("#apply_price"),
      (e) => {
        e.addEventListener("click", (elem) => {
          elem.preventDefault();
          elem.target.parentElement.style.display = "none";
          ...
          a.dataset.precio =
            a.parentElement.querySelector("#price_per_second").value;
          a.innerHTML = formatearNumeroConComas(
            a.dataset.precio * a.dataset.time,
          );
          proyectos_objetos_row();
        });
      },
...
function proyectos_objetos_row() {
  if (document.querySelector(".proyectos_objetos")) {
    ...
    let enabled = false;
    Array.prototype.forEach.call(
      document.querySelectorAll(".proyectos_objetos"),
      (e) => {
        enabled = false;
    ...
    document.querySelector("#total_price").innerHTML =
      formatearNumeroConComas(total_price);
    document.querySelector("#total_price_enabled").innerHTML =
      formatearNumeroConComas(total_price_enabled);
  }
}
